...
Route::get('/', 'CatalogoController@getBienvenida')
    ->name('home')
    ->middleware('usuarioBloqueado');

/* CATALOGO */
Route::group(
    ['prefix' => 'catalogo/'], function () {
        // Catálogo y bienvenida son la misma página
        Route::get('', 'CatalogoController@getBienvenida');
        // VER OBRA
        Route::get('ver/{titulo_obra}', 'CatalogoController@getVer')
            ->name('ver-obra');
        // NUEVA OBRA
        Route::get('nueva', 'CatalogoController@getNueva')
            ->middleware('admin');
...
Route::get('activacion', 'Auth\RegisterController@getActivacion')
    ->name('activacion');
Route::post('activacion', 'Auth\RegisterController@postActivacion');

/* CONFIRMACIÓN DE USUARIO */
Route::get(
    'auth/confirm/email/{email}/confirm_token/{confirm_token}',
    'Auth\RegisterController@confirmRegister'
);

/* FAVORITOS */
Route::get('/favorito', 'FavoritoController@getFavorito')
    ->name('favorito')
    ->middleware('auth');
...
Route::bind(
    'obra', function ($titulo) {
        return anayzquierdo\Obra::where('titulo_obra', $titulo)->first();
    }
);

Route::group(
    ['prefix' => 'carrito/'], function () {
        Route::get('ver', 'CarritoController@verCarrito')
            ->name('ver-carrito')
            ->middleware('auth');
        Route::get('add/{obra}', 'CarritoController@addCarrito')
            ->name('add-carrito')
            ->middleware('auth');
        Route::get('borrar/{obra}', 'CarritoController@borrarCarrito')
            ->name('borrar-carrito')
            ->middleware('auth');
        Route::get('vaciar', 'CarritoController@vaciarCarrito')
            ->name('vaciar-carrito')
            ->middleware('auth');
    }
);

Route::get('detalle_pedido', 'CarritoController@getDetallePedido')
    ->name('detalle-pedido')
    ->middleware('auth');

/* PAYPAL */
// Enviamos nuestro pedido a paypal
Route::get('pago', 'PaypalController@postPago')
    ->name('pago-paypal')
    ->middleware('auth');

// Paypal redirecciona a esta ruta
Route::get('pago/estado', 'PaypalController@getPagoEstado')
    ->name('pago-estado');
