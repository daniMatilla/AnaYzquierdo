...
{{-- PAGINACION OBRAS --}}
  @php
      $currentPage = $obras->currentPage(); //Página actual
      $maxPages = $currentPage + 3; //Máxima numeración de páginas
      $firstPage = 1; //Primera página
      $lastPage = $obras->lastPage(); //Última página
      ...
      <ul class="pagination center">
        ...
        <li class="{{ ($currentPage == $firstPage) ? 'disabled' : 'waves-effect waves-teal' }}">
          <a href="{{ ($currentPage > 1) ? $obras->url($firstPage) : '#!' }}">Primera</a>
        </li>

        <!-- Botón para navegar a la página anterior -->
        <li class="{{ ($currentPage == $firstPage) ? 'disabled' : 'waves-effect waves-teal' }}">
          <a href="{{ ($currentPage > 1) ? $obras->url($forwardPage) : '#!' }}">
        ...
        <li class="{{ ($currentPage == $lastPage) ? 'disabled' : 'waves-effect waves-teal' }}">
          <a href="{{ ($currentPage < $lastPage) ? $obras->url($nextPage) : '#!' }}">
        ...
        <li class="{{ ($currentPage == $lastPage) ? 'disabled' : 'waves-effect waves-teal' }}">
          <a href="{{ ($currentPage < $lastPage) ? $obras->url($lastPage) : '#!' }}">Última</a>
        </li>
      </ul>
      {{-- FIN PAGINACION --}}
    </div>